...
// Cria o registro de um usuario novo com teste gratis automatico de 1 dia
function dadosTrialNovo(email) {
  const validoAte = new Date();
  validoAte.setDate(validoAte.getDate() + 1);
  return {
    email,
    status: 'pago',
    plano: 'teste_1dia',
...
// Admins sempre passam. Usuario novo ganha 1 dia de teste gratis automatico.
async function verificarAssinatura(req, res, next) {
  ...
  try {
    ...
    if (!doc.exists) {
      ...
      return next(); // primeiro dia liberado na hora, sem precisar de aprovacao
    }
